...
_bootstrap_module("jira_integration.py")
_bootstrap_module("firebase_utils.py")
...
class LogiPanApp(SlackIntegrationMixin, JiraIntegrationMixin, FirebaseUtilsMixin):
    def __init__(self, root):
        ...
        # 화면의 65% 정도 (최대는 1080x800)
        width = min(1080, int(sw * 0.65))
    ...
    def _self_update_check(self):
        ...
        try:
            ...
            # 1-2. oauth_client.json 다운로드 (없을 때만)
            oauth_path = os.path.join(script_dir, "oauth_client.json")
            if not os.path.exists(oauth_path):
                try:
                    oauth_url = f"{base_url}/oauth_client.json"
                    urllib.request.urlretrieve(oauth_url, oauth_path)
                    print("✅ oauth_client.json 다운로드 완료")
                except Exception as e:
    ...
    def setup_jira_waiting(self):
        ...
        self._jira_tree.column("status", width=60, anchor="center")
        self._jira_tree.column("key", width=90, anchor="center")
        self._jira_tree.column("summary", width=250, anchor="w")
        self._jira_tree.column("reporter", width=90, anchor="center")
        self._jira_tree.column("created", width=80, anchor="center")
...
# 3. 데이터 목록 갱신 엔진 (오류 수정 및 새 댓글 감지)
    ...
    def open_stocktake_detail_popup(self, session_id):
        ...
        # [추가] 색상 범례 (한 줄, 가로 스크롤 없이 한눈에)
        legend_row = tk.Frame(left, bg="white")
        legend_row.pack(fill="x", pady=(0, 4))
        tk.Label(legend_row, text="범례:", bg="white",
                 font=("맑은 고딕", 8, "bold"), fg="#555").pack(side="left", padx=(0, 4))
        for txt, color in [
            (" 완료 ",  "#D1FAE5"),
            (" 출고 ",  "#DBEAFE"),
            (" 혼합 ",  "#E0E7FF"),
            ...
            tk.Label(legend_row, text=txt, bg=color, fg="#333",
                     font=("맑은 고딕", 8), bd=1, relief="solid").pack(side="left", padx=1)
        ...
        tree.tag_configure("force_done", background="#D1D5DB")   # 진한 회색 (임의 완료)
        ...
        def on_tree_complete_selected():
            ...
            try:
                ...
                for key, it in targets:
                    ...
                    # remaining=0 + force_completed 플래그 같이 저장 (임의완료 추적)
                    item_ref = self._rtdb_ref(f"stocktake_data/{session_id}/items/{key}")
                    item_ref.update({
                        "remaining": 0,
                        "force_completed": True,
                        "force_completed_at": ts_ms,
                    })
                    state["items"][key]["remaining"] = 0
                    state["items"][key]["force_completed"] = True
                    state["items"][key]["force_completed_at"] = ts_ms
                    applied_log.append({
                        "loc": it.get("loc", ""),
                        "bc": it.get("bc", ""),
                        "from": old_remaining,
            ...
            try:
                ...
                for idx, row in df.iterrows():
                    ...
                    from_updates = {
                        ...
                        "qty_moved_out": prev_moved_out + qty,
                        "moved_at": ts_ms,
                    }
        ...
        def render_items_table():
            ...
            for key, it in sorted_items:
                ...
                qty_moved_in = it.get("qty_moved_in", 0)
